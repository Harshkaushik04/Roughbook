...
// Text Editor
const textEditor = getId('textEditor');
const textEditorHeader = getId('textEditorHeader');
const textEditorBtn = getId('textEditorBtn');
const textEditorElement = getId('textEditorElement');
const textEditorCloseBtn = getId('textEditorCloseBtn');
...
function initClientPeer() {
    ...
    const newTitle = 'Rough book | Room';
    document.title = newTitle;
    const clientTitle = getId('clientTitle');
    if (clientTitle) clientTitle.innerHTML = newTitle;
...
function handleButtonsRule() {
    ...
    displayElements([
        ...
        { element: textEditorBtn, display: buttons.main.showTextEditorBtn }, // ADD THIS LINE
...
function manageButtons() {
    ...
    setChatEmojiBtn();
    setMyWhiteboardBtn();
    setTextEditorBtn(); // ADD THIS LINE
    setSnapshotRoomBtn();
    setMyFileShareBtn();
    setDocumentPiPBtn();
    setMySettingsBtn();
...
function setTextEditorBtn() {
    ...
    // Toggle text editor
    textEditorBtn.addEventListener('click', (e) => {
        handleTextEditorToggle();
    });
    // Close text editor
    textEditorCloseBtn.addEventListener('click', (e) => {
        handleTextEditorToggle();
    });

    // Real-time text sync listener
    textEditorElement.addEventListener('input', (e) => {
        // Send text data to peers
        const text = e.target.value;
        sendToServer('textEditorUpdate', {
            room_id: roomId,
            peer_name: myPeerName,
            text: text,
        });
...
function handleTextEditorToggle() {
    ...
    thereArePeerConnections() ? textEditorAction(getTextEditorAction('toggle')) : toggleTextEditor();
}
...
function toggleTextEditor() {
    ...
    if (!isTextEditorOpen) {
        ...
        setTippy(textEditorBtn, 'Open the Text Editor', placement);
    }
    textEditor.classList.toggle('show');
    isTextEditorOpen = !isTextEditorOpen;
    // Adapt video grid layout
    resizeVideoMedia(); 
}
...
function textEditorAction(config) {
    ...
    handleTextEditorAction(config, false);
}

/**
 * Text Editor: handle actions
 * @param {object} config data
...
function showAbout() {
    ...
    Swal.fire({
        ...
        html: `
            <br/>
            <div id="about">
                <p>A blank page for your ideas.</p>
                <p>Powered by WebRTC</p>
                <br /><br />
                <hr />
                <span>&copy; 2025 Rough book, all rights reserved</span>
                <hr />
            </div>
        `,
        showClass: { popup: 'animate__animated animate__fadeInDown' },
